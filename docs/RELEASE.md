...
The static release is generated with `npm run build`; `npm run check` validates the release boundary. `build-manifest.json` identifies the product, date, routes and narrative balance. `release-files.json` is the exact public allowlist. The production deployment uploads only that built output and a static Vercel configuration, not the full repository or private attachments.
